...
class Card:
    ...
    def value_for_sorting(self):
        # Number card (2-10)
        val = self.value
        if self.value not in get_all_face_values():
            return int(val)
...
# todo add functionality for checking the kickers also
# todo might need to add more instance vars
class RankObject:
    def __init__(self, ranking, top_card, kicker1=None, kicker2=None, kicker3=None):
    ...
    def __gt__(self, other):
        if ranking_map[self.ranking] > ranking_map[other.value_for_sorting]:
            return top_card_map[self.top_card] > top_card_map[other.top_card]
        return False
    ...
    def __eq__(self, other):
        return ranking_map[self.ranking] == ranking_map[other.value_for_sorting] \
               and top_card_map[self.top_card] == top_card_map[other.top_card]
